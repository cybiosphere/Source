...
:CSensor(totalNbFocusObjects * VIEW_SIZE_PER_FOCUS, tWeightRate, UID_BASE_SENS_VIEW_FAR + angle )
{
  m_pBrain = pBrain;
...
//---------------------------------------------------------------------------
const std::vector<sensorValType>& CSensorViewFar::UpdateAndGetStimulationTable()
{
  int i;
                                     ...
                                     int direction)
{
  ...
  for (i = 0; i < biotopFoundIds.nbFoundIds; i++)
  {
    ...
    if (pCurEntity->getId() != m_pBrain->getBrainFocusedEntityId())
    {
    ...
    if (testChance(viewChance))
    {
      ...
      for (j = 0; j < VIEW_SIZE_PER_FOCUS; j++)
      {
        ...
        m_pEntityViewFarTab[i].computedWeight += m_pEntityViewFarTab[i].weightTab[j] * m_tBonusRate[stimulationTabOffset+j];
      }
  ...
  for (j = 0; j < maxNumFocusObject; j++)
  {
    ...
    m_pBrain->proposeNewFocusedEntityCandidate(m_pEntityViewFarTab[maxWeightViewTabIndex].pEntity, maxComputedWeight, 0,
