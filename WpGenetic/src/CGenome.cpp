...
bool CGenome::saveInFile(string fileNameWithPath)
{
  string fileString = "";
  bool resu = false;
  char* pbuf = NULL;
  ...
  string strSection,strData,strName;
  CChromosome* pCurChro = NULL;

  strData = FormatString("%2d",getClass());
  resu = writeStringSection("GENOME","ClassName",getOwnClassStrName(),fileString);
  resu = writeStringSection("GENOME","ClassId",strData,fileString);
  resu = writeStringSection("GENOME","Specie",getSpecieName(),fileString);
  strData = FormatString("%2d",m_bSexualDimorphism);
  resu = writeStringSection("GENOME","Dimorphism",strData,fileString);
  strData = FormatString("%2d",getNumPair());
  resu = writeStringSection("GENOME","NbPairs",strData,fileString);
  ...
  {
    strSection = FormatString("PAIR_%02d",i);

    strData = FormatString("%d",getPair(i)->getSex());
    writeStringSection(strSection,"Type",strData,fileString);

    pCurChro = getPair(i)->getMaterChromosome();
    strData = FormatString("%d",pCurChro->getNumGene());
    writeStringSection(strSection,"NumberGeneM",strData,fileString);
    for (j=0; j<pCurChro->getNumGene(); j++)
    {
      strData = pCurChro->getGene(j)->buildStringDataFromGene();
      strName = FormatString("GeneM%d",j);
      writeStringSection(strSection,strName,strData,fileString);
    }

    pCurChro = getPair(i)->getPaterChromosome();
    strData = FormatString("%d",pCurChro->getNumGene());
    writeStringSection(strSection,"NumberGeneP",strData,fileString);
    for (j=0; j<pCurChro->getNumGene(); j++)
    {
      strData = pCurChro->getGene(j)->buildStringDataFromGene();
      strName = FormatString("GeneP%d",j);
      writeStringSection(strSection,strName,strData,fileString);
    }
  }
...
bool CGenome::loadFromFile(string fileNameWithPath)
{
  string fileString;
  char* pbuf = NULL;
  unsigned long int begin, end;
  ...
  char resuStr[10000];
  string specieName;
  ClassType_e classType;
  int nbPairs,indexPair,cpt;
  DWORD readLen = 0;
  int i,j,geneIndex;
  CGene* pGene;
  CChromosome* pCurChro;

  resu = getStringSection("GENOME","ClassId","00",resuStr,10,fileString);
  classType = (ClassType_e)atoi(resuStr);
  resu = getStringSection("GENOME","Specie","",resuStr,1000,fileString);
  specieName = resuStr;
  resu = getStringSection("GENOME","Dimorphism","0",resuStr,10,fileString);
  m_bSexualDimorphism = (bool)atoi(resuStr);
  resu = getStringSection("GENOME","NbPairs","0",resuStr,10,fileString);
  nbPairs = (ClassType_e)atoi(resuStr);
  ...
  for (i=0; i<nbPairs; i++)
  {
    ...
    getStringSection(strSection,"Type","",resuStr,1000,fileString);
    switch(atoi(resuStr))
    {
    case CHROMOSOME_SEX_MALE:
      setSexMale(i);
    ...
    getStringSection(strSection,"NumberGeneM","0",resuStr,1000,fileString);
    ...
    for (j=0; j<cpt; j++)
    {
      ...
      {
        strName = FormatString("GeneM%d",j);
        readLen = getStringSection(strSection,strName,"",resuStr,10000,fileString);
        strData = resuStr;
        pGene = pCurChro->getGene(geneIndex);
        pGene->buildGeneFromStringData(strData);
      }
    }

    getStringSection(strSection,"NumberGeneP","0",resuStr,1000,fileString);
